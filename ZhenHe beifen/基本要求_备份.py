...
class EnterpriseMatchingSystem:
        ...
        for idx, row in self.template_df.iterrows():
            ...
            if pd.notna(row.iloc[3]) and pd.notna(row.iloc[4]):
                temp_table.append({
                    ...
                    'credit_code': str(row.iloc[4]).strip(),
                    'template_row': idx,
                    'source': '在线监测'
                })
    ...
    def exact_match(self, target_companies: List[Dict], source_companies: List[Dict]) -> Tuple[List[Tuple], List[Dict]]:
        ...
        for temp_company in target_companies:
            ...
            for source_idx, source_company in enumerate(unmatched_source):
                ...
                if (clean_temp_name == clean_source_name or 
                    clean_temp_name in clean_source_name or 
                    clean_source_name in clean_temp_name):
                    matched_pairs.append((temp_company, source_company))
                    temp_matched.append(source_idx)
                    break
    ...
    def batch_llm_verify_match(self, verification_batch: List[Tuple[Dict, List[Tuple[Dict, float]]]]) -> List[Optional[Dict]]:
        ...
        try:
            ...
            batch_prompt += "**重要规则**: \n"
            batch_prompt += "1. 地区名称不同（如'荣县' vs '成都'）绝对不是同一家公司\n"
            batch_prompt += "2. 核心业务名称相同或高度相似才可能是同一家公司\n"
            batch_prompt += "3. 注意简称、全称、括号内容等不同表示方式\n\n"
    ...
    def _log_candidates(self, verification_batch: List[Tuple[Dict, List[Tuple[Dict, float]]]]):
        ...
        try:
            ...
            log_file = os.path.join(log_dir, f"候选者匹配日志_{timestamp}.txt")
            
            with open(log_file, 'a', encoding='utf-8') as f:
                f.write(f"=" * 80 + "\n")
                f.write(f"候选者匹配日志 - 时间: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write(f"总匹配组数: {len(verification_batch)}\n")
                f.write(f"=" * 80 + "\n\n")
                ...
                for i, (target_company, candidate_companies) in enumerate(verification_batch):
                    ...
                    for j, (candidate, score) in enumerate(candidate_companies):
                        ...
                        f.write(f"    相似度: {score:.4f}\n")
                        if 'source_file' in candidate:
                            f.write(f"    来源文件: {candidate['source_file']}\n")
                        f.write(f"\n")
        ...
        if unmatched_source and self.embedding_model:
            ...
            for batch_idx, i in enumerate(range(0, len(verification_batch), batch_size)):
                ...
                for j, (temp_company, candidates) in enumerate(batch):
                    if i+j < len(batch_results) and batch_results[i+j]:
                        matched_company = batch_results[i+j]
...
def main():
    """主函数"""
    # 文件路径配置
    template_path = "/home/wjw/diting/ZhengHe/xingxuqiu/四川环科院结果模板1.xlsx"
    output_path = "/home/wjw/diting/ZhengHe/xingxuqiu/四川环科院结果模板1_匹配结果15.xlsx"
    
    # 阿里云DashScope API密钥（请替换为您的实际密钥）
    dashscope_api_key = os.getenv('DASHSCOPE_API_KEY', 'your-dashscope-api-key-here')
    
    file_paths = {
        '用电监控': '/home/wjw/diting/ZhengHe/xingxuqiu/前100行数据/企业电监控档案导出_前100行.xlsx',
        '排污许可': '/home/wjw/diting/ZhengHe/xingxuqiu/前100行数据/排污许可证涉气企业_前100行.xlsx',
        '减排清单': '/home/wjw/diting/ZhengHe/xingxuqiu/前100行数据/减排清单_前100行.xlsx',
        '源清单': '/home/wjw/diting/ZhengHe/xingxuqiu/前100行数据/源清单_前100行.xlsx'
    }
